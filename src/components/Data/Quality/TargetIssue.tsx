...
class TargetIssue extends Component<TargetIssueProps> {
  ...
  render() {
    ...
    const targetPercent = {
      missing: nullCount === 0 ? 0 : (nullCount * 100 / (totalRawLines || 1)) < 0.01 ? "<0.01" : formatNumber((nullCount * 100 / (totalRawLines || 1)).toString(), 2),
      mismatch: mismatchCount === 0 ? 0 : (mismatchCount * 100 / (totalRawLines || 1)) < 0.01 ? "<0.01" : formatNumber((mismatchCount * 100 / (totalRawLines || 1)).toString(), 2),
      outlier: outlierCount === 0 ? 0 : (outlierCount * 100 / (totalRawLines || 1)) < 0.01 ? "<0.01" : formatNumber((outlierCount * 100 / (totalRawLines || 1)).toString(), 2)
    }
