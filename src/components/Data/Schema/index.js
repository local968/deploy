  ...
  formatTable = () => {
    ...
    for (let i = 0; i < realColumn; i++) {
      ...
      if (i === index.columnHeader - 1) {
        ...
        const canTransforToCategorical = this.props.projectStore.project.stats[key].originalStats.doubleUniqueValue < Math.min(this.props.projectStore.project.stats[key].count * 0.1, 1000)
        const colValue = this.dataType[key]
        selectData.content = <select value={colValue} onChange={this.select.bind(null, key)}>
          {!canTransforToCategorical && <option value="Raw">{EN.Categorical}.</option>}
          {canTransforToCategorical && <option value="Categorical">{EN.Categorical}</option>}
          <option value="Numerical">{EN.Numerical}</option>
        </select>
        selectData.title = { Numerical: EN.Numerical, Categorical: EN.Categorical, Raw: EN.Categorical + '.' }[colValue]
        if (target && target === key) {
          selectData.cn = classnames(styles.cell, styles.target);
          selectData.content = <span>{{ Numerical: EN.Numerical, Categorical: EN.Categorical, Raw: EN.Categorical + '.' }[colValue]}</span>
        }
