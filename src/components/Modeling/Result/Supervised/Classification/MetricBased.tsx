...
const Based = (props: MetricBasedProps) => {
  ...
  return (
    <div className={styles.main}>
      <Show name="result_MetricCorrection">
        <div className={styles.button} onClick={onShow}>
          <span>
            {EN.MetricCorrection}
            <Hint content={EN.MetricCorrectionHint} />
          </span>
        </div>
        <Modal
          visible={visible}
          width={'1200px'}
          content={<MetricBased {...props} onClose={onClose} />}
          title={EN.MetricBased}
          onClose={onClose}
          closeByMask={false}
          showClose={true}
        />
      ...
      case 'recall':
        ...
        return (
          <div className={styles.condition}>
            ...
            <Select
              onChange={setType}
              style={{ width: 140, margin: '0px 10px' }}
              value={state.type}
            >
              {recallList.map((v, k) => {
                return (
  ...
  return (
    <section className={styles.section}>
            ...
            return (
              ...
              >
                <input
                  ...
                  type="radio"
                  checked={checked}
                  onChange={setMetric(v.value)}
                />
                <label htmlFor={`metric${k}`}>{`${v.label}${
                  v.value === 'default' ? `(${EN.Default})` : ``
                }`}</label>
      ...
      <footer className={styles.footer}>
        <div className={styles.metricButton}>
          <button onClick={loading ? () => {} : onSave}>
            <span>{loading ? <Icon type="loading" /> : EN.confirm}</span>
          </button>
        </div>
        <div className={styles.metricButton}>
          <button
        ...
        <div className={styles.metricButton}>
          <button onClick={loading ? () => {} : onReset}>
            <span>{EN.Reset}</span>
          </button>
        </div>
      </footer>
