...
const init = (server, sessionParser) => {
  ...
  wss.on('connection', (socket, req) => {
    ...
    const log = `${ip} - ${req.session.userId} - [${moment().format()}] - connected - ${req.headers['user-agent'] || ''}`
    ...
    socket.on('close', (socket, code, reason) => {
      ...
      const log = `${ip} - ${req.session.userId} - [${moment().format()}] - closed - ${req.headers['user-agent'] || ''}`
      redis.set(`log:${moment().format('YYYY:MM:DD')}:${uuid.v4()}}`, log)
      // client closed
