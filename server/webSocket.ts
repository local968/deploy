...
import { saveMessage, removeMessage } from './reboot';
...
const init: any = (server, sessionParser) => {
  const wss: any = new WebSocket.Server({
    verifyClient: (info, done) =>
      sessionParser(info.req, {}, () => done((info.req as any).session.userId)),
    server,
    path: '/ws',
  });
  ...
  wss.on('connection', (socket: any, req: any) => {
    ...
    const log = `${ip} - ${
      req.session.userId
    } - [${moment().format()}] - ${moment().valueOf()} - connected - ${req
      .headers['user-agent'] || ''}`;
    ...
    socket.on('close', (socket, code, reason) => {
      ...
      const log = `${ip} - ${
        req.session.userId
      } - [${moment().format()}] - ${moment().valueOf()} - closed - ${req
        .headers['user-agent'] || ''}`;
      redis.set(`log:${moment().format('YYYY:MM:DD')}:${uuid.v4()}}`, log);
      // client closed
